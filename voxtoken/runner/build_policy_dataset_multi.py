...
def build_policy_dataset_multi(
    *,
    manifest_jsonl: Path,
    out_jsonl: Path,
    cfg: Dict[str, Any],
    ...
    out_jsonl.write_text("".join(s + "\n" for s in out_rows), encoding="utf-8")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a multi-case policy dataset from CT-RATE TS GT boxes.")
    ...
    parser.add_argument("--split", required=True, help="Split name to include (train|val|test)")
    ...
    try:
        summary = build_policy_dataset_multi(
