...
def train_evidence_head(cfg: Dict[str, Any]) -> None:
    ...
    (run_dir / "checkpoint.json").write_text(json.dumps({"stage": "E", "run_id": run_id}, ensure_ascii=False), encoding="utf-8")
...
def main() -> None:
    ...
    cfg = dict(cfg or {})
    train_evidence_head(cfg)
    print(json.dumps({"status": "ok", "out_dir": str(cfg.get("out_dir", "outputs/train_evidence"))}, ensure_ascii=False))
